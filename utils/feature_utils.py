...
def gabor_preprocess(frame):
    ...
    kernel = cv2.getGaborKernel((9,9),4.0,np.pi/4,10.0,0.5)
...
def fuzzy_c_means_intensity(img,c=3,iters=8):
    ...
    for _ in range(iters):
        ...
        centers = (mem*x[:,None]).sum(axis=0)/mem.sum(axis=0)
...
def extract_features_from_frame(frame,cfg):
    ...
    feats = np.array(
        rgb+[lum_mean,lum_std,var,edge,dct_total,dct_ratio],
        dtype=np.float32
    )
    ...
    if cfg["use_fcm"]:
        ...
        centers = fuzzy_c_means_intensity(pre,cfg["fcm_k"])
